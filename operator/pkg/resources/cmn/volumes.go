...
// Explicitly define file permissions as the K8s defaults to avoid causing rollout on diff
var (
	CMDefaultMode     = corev1.ConfigMapVolumeSourceDefaultMode
...
func NewAISVolumes(ais *v1beta1.AIStore, daeType string) []corev1.Volume {
	...
	} else if hostpathPrefix := ais.Spec.StateStorageHostPathPrefix(); hostpathPrefix != nil {
		// hostpath: no dynamic storage class or emptyDir provided; use the host directory
		volumes = append(volumes, corev1.Volume{
			Name: stateVolume,
			VolumeSource: corev1.VolumeSource{
