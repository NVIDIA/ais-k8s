...
if [[ -n "$AIS_CONFIG" ]]; then
    ...
    _cfg_set SKIP_ETHTOOL     '.skip.ethtool'
fi
...
SKIP_ETHTOOL="${SKIP_ETHTOOL:-false}"
...
check_dependencies() {
    ...
    if [[ "$SKIP_ETHTOOL" != "true" ]] && ! command -v ethtool &>/dev/null; then
        missing+=("ethtool")
    fi
...
# =============================================================================
# 3. Ethtool NIC tuning (driven by YAML config, persisted via cloud-init per-boot)
# =============================================================================
configure_ethtool() {
    [[ "$SKIP_ETHTOOL" == "true" ]] && { log "Skipping ethtool (SKIP_ETHTOOL=true)"; return 0; }
    [[ -z "$AIS_CONFIG" ]] && { log "No AIS_CONFIG provided, skipping ethtool"; return 0; }

    local lines
    lines=$(yq -r '.ethtool // [] | .[]' "$AIS_CONFIG")
    [[ -z "$lines" ]] && { log "No ethtool config, skipping"; return 0; }

    local cmds=""
    while IFS= read -r item; do
        [[ -z "$item" ]] && continue
        cmds+="apply ethtool $item"$'\n'
    done <<< "$lines"

    local script=/var/lib/cloud/scripts/per-boot/10-ais-ethtool.sh
    mkdir -p "$(dirname "$script")"

    cat > "$script" <<EOF
#!/bin/bash
# Auto-generated by ais_host_config.sh — do not edit by hand.
set -euo pipefail

# exit 80: ethtool "already at requested value" — not an error
apply() { "\$@" || [ \$? -eq 80 ]; }
...
EOF
    ...
    log "wrote $script, applying now"
    bash "$script"
}

# =============================================================================
# 4. Data filesystems (delegates to ais_datafs.sh)
# =============================================================================
configure_datafs() {
