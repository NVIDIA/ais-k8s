...
This tool provides an easy way to back up and restore internal config data and metadata in an AIS cluster for recovery or rollback purposes. It also supports cleaning PVs (Persistent Volumes) of AIS state and/or data.
...
Backup and restore work by checking for existing PVCs that are used for mounting AIS state. 
