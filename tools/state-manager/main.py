...
def backup(manager: K8sManager):
    ...
    pod_config = PodConfig(
        ...
        labels={
            "app.kubernetes.io/name": f"{manager.cluster_name}-backup"
        }
    )
...
def delete(manager: K8sManager, metadata: List[AISMetadata]):
    ...
    pod_config = PodConfig(
        ...
        exec_cmd="",
        labels={
            "app.kubernetes.io/name": f"{manager.cluster_name}-restore"
        }
    )
...
def create_arg_parser():
    parser = argparse.ArgumentParser(description='Manage AIS cluster metadata backups, deletions, and restores')
    ...
    parser.add_argument('--action', type=str, choices=['backup', 'delete', 'restore'],
                        help='Action to perform (backup, delete, restore)')
    parser.add_argument('--delete-md', type=str,
                       help=f'Comma-separated metadata types ({AISMetadata.get_options_str()}) for deletion')
    parser.add_argument('--restore-src', type=str,
                       help='Source backup tgz file to restore')
    return parser
...
def prompt_missing(args):
    ...
    if not args.action:
        while True:  # Keep asking until valid action
            action_input = input("Enter action (backup/delete/restore): ").strip().lower()
            if action_input in ['backup', 'delete', 'restore']:
                args.action = action_input
                break
            print("Error: Invalid action. Please enter 'backup', 'delete', or 'restore'.")
